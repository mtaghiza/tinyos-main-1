...
yl <- "P [0, 1.0]"
t <- "Root->Leaf PRR"
plotCdf(ds, xl, yl, t, plotPdf, paste(outPrefix, 'prr_rl','pdf', sep='.'))
...
yl <- "P [0, 1.0]"
t <- "Leaf->Root PRR"
plotCdf(ds, xl, yl, t, plotPdf, paste(outPrefix, 'prr_lr', 'pdf', sep='.'))

ds <- x$src_to_dest - x$dest_to_src
xl <- "PRR (root,leaf) - (leaf, root)"
yl <- "P [0, 1.0]"
t <- "PRR asymmetry"
plotCdf(ds, xl, yl, t, plotPdf, paste(outPrefix, 'prr_asym', 'pdf', sep='.'))
