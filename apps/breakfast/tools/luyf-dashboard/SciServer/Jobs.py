...
def getDockerJobsListQuick(top=10, open=None, start=None, end=None,labelReg=None):
    """
    ...
    :return: a pandas datafranme, each row containing information about a submitted job.
    :raises: Throws an exception if the HTTP request to the Authentication URL returns an error, and if the HTTP request to the JOBM API returns an error.
    ...
    if token is not None and token != "":
        ...
        j['data']=j.pop('rows')
        j.pop('hrNames')
        df=pandas.read_json(json.dumps(j), orient='split')
